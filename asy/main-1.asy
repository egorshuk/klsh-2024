...
size(11.4cm);
triangle t=triangleabc(8, 9, 11);
draw(t, linewidth(1.4*bp));
point P = 0.5*t.B+0.35*(t.C-t.B); dot(P, orange);
point Q = isogonalconjugate(t, P); dot(Q, brown);
...
perpendicularmark(t.AB, p_c, size=7, blue); perpendicularmark(t.AB, q_c, size=7, deepgreen);
perpendicularmark(t.BC, p_a, size=7, blue); perpendicularmark(t.BC, q_a, size=7, deepgreen);
perpendicularmark(reverse(t.AC), p_b, size=7, blue); perpendicularmark(reverse(t.AC), q_b, size=7, deepgreen);

circle G = circle(P_a, P_b, P_c); draw(G, 1.2*bp+mediumred+dashed);
...
markangle(n = 1, line(pa), line(t.AC), radius=1cm, orange);
markangle(n = 1, line(t.AB), line(qa), radius=.8cm, brown);

markangle(n = 2, rotate(180)*line(t.AC), line(pc), radius=1cm, orange);
markangle(n = 2, line(qc), line(t.BC), radius=.8cm, brown);

markangle(n = 3, line(t.BC), line(qb), radius=1cm, brown);
markangle(n = 3, line(pb), line(t.AB), radius=.8cm, orange);

triangle t_P = pedal(t, P); draw(t_P, royalblue);
triangle t_Q = pedal(t, Q); draw(t_Q, deepgreen);

//segment pq = segment(P, Q);
//point M = midpoint(pq); dot(M, purple); draw(pq, purple, //StickIntervalMarker(2,1, size=7));
...
//perpendicularmark(t.AB, m_c, size=7, purple); //perpendicularmark(reverse(t.AC), m_b, size=7, purple);
//perpendicularmark(t.BC, m_a, size=7, purple);
